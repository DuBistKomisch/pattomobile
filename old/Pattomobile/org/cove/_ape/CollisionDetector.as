...
package org.cove.ape {
	...
	public final class CollisionDetector {	
		...
		internal static function test(objA:AbstractParticle, objB:AbstractParticle):void {	
			...
			if (! (objA.visible && objB.visible)) return;
		 ...
		 */
		public static function testOBBvsOBB(ra:RectangleParticle, rb:RectangleParticle):void {
			...
			ra.dispatchEvent(new CollisionEvent(rb.type == null ? "" : rb.type, rb, ra))
			rb.dispatchEvent(new CollisionEvent(ra.type == null ? "" : ra.type, ra, rb))
		 ...
		 */
		public static function testOBBvsCircle(ra:RectangleParticle, ca:CircleParticle):void {
			...
			ra.dispatchEvent(new CollisionEvent(ca.type == null ? "" : ca.type, ca, ra))
			ca.dispatchEvent(new CollisionEvent(ra.type == null ? "" : ra.type, ra, ca))
		 ...
		 * for handling.
		 */	
		public static function testCirclevsCircle(ca:CircleParticle, cb:CircleParticle):void {
			...
			if (collisionDepth > 0) {
				ca.dispatchEvent(new CollisionEvent(cb.type == null ? "" : cb.type, cb, ca))
				cb.dispatchEvent(new CollisionEvent(ca.type == null ? "" : ca.type, ca, cb))
				collisionNormal.divEquals(mag);
