	...
	public class AIRBadge extends MovieClip {
		...
		public function AIRBadge() {
			...
			try {
				...
				if (_buttonColor != "transparent") _buttonColor = validateColor(_buttonColor);
				...
				if ( _appName == null || _appName.length == 0 || _appName.indexOf("<") >= 0) _appName = null;
			} catch (error:Error) {
			...
			root.statusMessage.mouseEnabled = false;
			...
			} else root.buttonBg_mc._visible = false;
			...
			} catch (e:Error) { root.statusMessage.text = e.message; }
		}
		...
		private function onButtonClicked(e:Event):void {
			try {
				switch (_air.getStatus()) {
					case "installed" :
						root.statusMessage.htmlText = "<p align='center'><font color='#" + _messageColor + "'>Download and select open to begin the installation.</font></p>";
						_air.installApplication( _appURL, _airVersion );
						break;
					case "available" :
						root.statusMessage.htmlText = "<p align='center'><font color='#" + _messageColor + "'>Starting install of AIR™ " + _airVersion + "...</font></p>";
						_air.installApplication( _appURL, _airVersion );
						break;
					case "unavailable" :
						// do nothing
		...
		private static function validateURL(url:String):String {
			if (url && url.length > 0) {
				...
				if (schemeMarker < 0) schemeMarker = url.indexOf("%3a");
				if (schemeMarker < 0) schemeMarker = url.indexOf("%3A");
				if (schemeMarker > 0) {
					var scheme:String = url.substr(0, schemeMarker).toLowerCase();
					if (scheme != "http" && scheme != "https" && scheme != "ftp") url = null;
				}
			}
			return url;
		}
		...
		private static function validateColor(color:String):String {
			if ( color == null || color.length != 6 ) color = "000000" else {
				var validHex:String = "0123456789ABCDEFabcdef";
				var numValid:int = 0;
				for (var i:int=0; i < color.length; ++i) if (validHex.indexOf(color.charAt(i)) >= 0) ++numValid;
